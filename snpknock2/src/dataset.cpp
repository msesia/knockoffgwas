...
Dataset::Dataset(const vector<Metadata>& _metadata, unsigned int _num_threads, unsigned int thinning) {
...
void Dataset::load_worker(unsigned int chr_min, unsigned int chr_max, unsigned int wid, vector<unsigned int> &progress) {  
  unsigned int n_steps = num_chrs_;
...
unsigned int Dataset::count_lines(string filename) const {
...
unsigned int Dataset::num_chrs() const {
