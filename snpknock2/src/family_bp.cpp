...
FamilyBP::FamilyBP(const vector<unsigned int>& _indices, const IbdCluster& ibd_cluster,
                   const vector <chaplotype>& _H,
                   const vector< vector<unsigned int > >& _ref,
...
void FamilyBP::sample_posterior(vector< vector<unsigned int> >& Z) {
  ...
  for(unsigned int i=0; i<num_haps; i++) {
    ...
    for(unsigned int j=0; j<num_snps; j++) {
      ...
      vector<unsigned int> neigh_diff_left, neigh_diff_right;
      if(is_junction) {
        if(j>0) {
      ...
      if(is_junction) {
        // Sample from posterior
        const unsigned int k = sample_posterior_joint(i,j,neigh_diff_left,neigh_diff_right);
        Z[i][j] = k;
        condition_on(i, j, k);
        for(unsigned int i2 : neighbors[i][j]) {
          Z[i2][j] = k;
  ...
  if(DEBUG) cout << "Finished sampling interior nodes from posterior. " << endl;
}

void FamilyBP::condition_on(const unsigned int i, const unsigned int j, const unsigned int k) {
  condition_on(i, j, k, false);
}

void FamilyBP::condition_on(const unsigned int i, const unsigned int j, const unsigned int k, const bool forward) {
  unsigned int i_abs = indices[i];
...
// void FamilyBP::get_marginals(const unsigned int i, const unsigned int j, vector<double>& f_j, vector<double>& marginals) const {
//   // Find absolute haplotype id
//   const unsigned int i_abs = indices[i];

//   // Find out which neighbors have just been added
//   vector<unsigned int> neighbors_diff_left;
//   if(j>0) {
//     if(neighbors[i][j] != neighbors[i][j-1]) {
//       std::set_difference(neighbors[i][j].begin(), neighbors[i][j].end(),
//                           neighbors[i][j-1].begin(), neighbors[i][j-1].end(),
...
//   vector<unsigned int> neighbors_diff_right;
//   if(j<(num_snps-1)) {
//     if(neighbors[i][j] != neighbors[i][j+1]) {
//       std::set_difference(neighbors[i][j].begin(), neighbors[i][j].end(),
//                           neighbors[i][j+1].begin(), neighbors[i][j+1].end(),
...
//   const unsigned int h_real = H[i_abs][j];
//   for(unsigned int k=0; k<K; k++) {
//     f_j[k] = emission_prob(h_real, H[ref[i_abs][k]][j], mut_rates[j]);
//   }
//   for(unsigned int i2 : neighbors[i][j]) {
//     const unsigned int i2_abs = indices[i2];
//     const unsigned int h2_real = H[i2_abs][j];
//     for(unsigned int k=0; k<K; k++) {
...
//   }
//   for(unsigned int i2 : neighbors_diff_left) {
//     for(unsigned int k=0; k<K; k++) {
//       marginals[k] *= messages_right[i2][j-1][k];
//     }
//   }
//   for(unsigned int i2 : neighbors_diff_right) {
//     for(unsigned int k=0; k<K; k++) {
...
int FamilyBP::run_bp(bool verbose) {
  ...
  while( (!converged)&& (it++ < it_max) ) {
    ...
    for(string dir : directions) {
      ...
      double diff_tmp = 0;
      for(unsigned int j : snp_idx) {
        for(unsigned int i : hap_idx) {
          ...
          vector<unsigned int> neigh_diff_left;
          vector<unsigned int> neigh_diff_right;
          if(is_junction) {
            if(j>0) {
              if(neighbors[i][j] != neighbors[i][j-1]) {
                std::set_difference(neighbors[i][j].begin(), neighbors[i][j].end(),
            ...
            //   for(unsigned int i2 : neigh_diff_left) cout << " " << i2;
            //   cout << endl;
            //   cout << " (i="  << i <<", j=" << j << "); neigh_diff_right:";
            //   for(unsigned int i2 : neigh_diff_right) cout << " " << i2;
            //   cout << endl;
            // }
          }
          ...
          if(dir == "right") {
            ...
            for(unsigned int k=0; k<K; k++) messages_right[i][j][k] = message[k];
          }
          if(dir == "left") {
            if(j<=0) continue;
            diff_tmp += send_messages_left(i, j, message, f_j, neigh_diff_left, neigh_diff_right);
            for(unsigned int k=0; k<K; k++) messages_left[i][j][k] = message[k];
          }
        }
      }
...
double FamilyBP::send_messages_right(const unsigned int i1, const unsigned int j, vector<double>& message, vector<double>& f_j,
                                     vector<unsigned int>& neigh_diff_left, vector<unsigned int>& neigh_diff_right) const {
  if(j>=(num_snps-1)) return(0);

  // Find absolute haplotype id
  const unsigned int i1_abs = indices[i1];

  // Pre-compute emissions probabilities
  const unsigned int h_real = H[i1_abs][j];
  for(unsigned int k=0; k<K; k++) {
    f_j[k] = emission_prob(h_real, H[ref[i1_abs][k]][j], mut_rates[j]);
  }
  for(unsigned int i2 : neighbors[i1][j]) {
    const unsigned int i2_abs = indices[i2];
    const unsigned int h2_real = H[i2_abs][j];
    for(unsigned int k=0; k<K; k++) {
  ...
  for(unsigned int l=0; l<K; l++) {
    ...
    for(unsigned int i2 : neigh_diff_left) new_const_msg *= messages_right[i2][j-1][l];
    for(unsigned int i2 : neigh_diff_right) new_const_msg *= messages_left[i2][j+1][l];
  ...
  for(unsigned int k=0; k<K; k++) {
    ...
    for(unsigned int i2 : neigh_diff_left) new_msg *= messages_right[i2][j-1][k];
    for(unsigned int i2 : neigh_diff_right) new_msg *= messages_left[i2][j+1][k];
...
double FamilyBP::send_messages_left(const unsigned int i1, const unsigned int j, vector<double>& message, vector<double>& f_j,
                                    vector<unsigned int>& neigh_diff_left, vector<unsigned int>& neigh_diff_right) const {
  if(j<=0) return(0);
...
bool FamilyBP::is_inactive(const unsigned int i, const unsigned int j) const {
...
unsigned int FamilyBP::sample_posterior_joint(const unsigned int i, const unsigned int j,
                                              const vector<unsigned int>& neigh_diff_left,
  ...
  for(unsigned int k=0; k<K; k++) {
    ...
    for(unsigned int i2 : neighbors[i][j]) {
      const unsigned int h2_real = H[indices[i2]][j];
    ...
    if(j>0) {
      ...
      for(unsigned int i2 : neigh_diff_left) weights[k] *= messages_right[i2][j-1][k];
    }
    if(j<(num_snps-1)) {
      weights[k] *= messages_left[i][j+1][k];
      for(unsigned int i2 : neigh_diff_right) weights[k] *= messages_left[i2][j+1][k];
    }
  }
  // Sample from posterior
  unsigned int k_star = weighted_choice(weights, rng);

  return(k_star);
}

void FamilyBP::print_Z(const vector< vector<unsigned int> >& Z) const {
