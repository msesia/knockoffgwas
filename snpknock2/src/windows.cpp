...
Windows::Windows(unsigned int _num_snps) {
  num_snps = _num_snps;
  num_windows = 1;
  start = {0};
  end = {num_snps};
...
void Windows::load(const vector<unsigned int>& start_points, const unsigned int _num_snps) {
  num_snps = _num_snps;
  ...
  for(unsigned int w=0; w<num_windows; w++) {
    ...
    for(unsigned int j=start.back(); j<end.back(); j++) {
      window[j] = w;
    }        
  } 
}
