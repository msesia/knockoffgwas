...
void Kinship::load_references_local(const vector<string>& ref_files) {
  ...
  for(unsigned int chr=0; chr<num_chrs; chr++) {
    ...
    references_local[chr].resize(num_haps, ivector2d(num_windows, vector<unsigned int>(K)));

    // Open file again
    fd_ref.open(ref_files[chr]);

    // Read body, skipping first two lines
    unsigned int i = 0;      // Haplotype index
    unsigned int w = 0;      // Window index
    unsigned int line = -2;  // File row, starting from file body
...
void Kinship::load_references_global(const vector<string>& ref_files) {
  ...
  for(unsigned int chr=0; chr<num_chrs; chr++) {
    ...
    references_global[chr].resize(num_haps, vector<unsigned int>(K));
...
Kinship::Kinship(const vector<Metadata>& metadata, const Covariates & covariates, vector<string> output_files,
                 unsigned int compression, unsigned int cluster_size_min, unsigned int cluster_size_max, unsigned int _K, unsigned int num_threads) {
  K = _K;
  ...
  references_global.resize(num_chrs);
  for(unsigned int chr=0; chr<num_chrs; chr++) {
    const unsigned int num_snps = metadata[chr].num_snps();
...
const ivector3d & Kinship::get_references(unsigned int chr) const {
  return(references_local[chr]);
}

const ivector2d & Kinship::get_references_global(unsigned int chr) const {
  return(references_global[chr]);
}

unsigned int Kinship::get_K() const {
  return(K);
}
